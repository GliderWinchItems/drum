...
void drum_items_init(struct DRUMSTUFF* p)
{
	...
	// Factor: Cable speed = drum_rpm * ((drum_dia * pi) / (gear_ratio * 60(sec/min) )
...
void drum_items_computespeed(struct DRUMENCODERCH* p)
{
	...
	do
	{ 
		p->tmp = p->cur; // Copy time and count set by ISR

		// Loop until re-read results in same two readings.
	} while ( (p->tmp.tim != p->cur.tim) || (p->tmp.cnt != p->cur.cnt) );
	...
	p->diff.cnt = p->tmp.cnt - p->prev.cnt;
	if (p->diff.cnt == 0)
	...
	p->Ccable_distance = p->Fcable_distance * (float)(p->cablezeroref + p->diff.cnt);
